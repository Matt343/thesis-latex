...
\usepackage{epsfig,endnotes,amsmath,pgf,hyperref,mathpartir}
...
\begin{mathpar}
\inferrule*{$$\texttt{A} \in \Gamma$$}{$$\Gamma \vdash \texttt{T<A>} <: \texttt{T<A>}$$}
\end{mathpar}
\item If \texttt{S} is covariant:
\begin{mathpar}
\inferrule*{$$(\texttt{A} <: \texttt{B}) \in \Gamma$$}{$$\Gamma \vdash \texttt{T<A>} <: \texttt{T<B>}$$}
\end{mathpar}

\item If \texttt{S} is contravariant:
\begin{mathpar}
\inferrule*{$$(\texttt{B} <: \texttt{A}) \in \Gamma$$}{$$\Gamma \vdash \texttt{T<A>} <: \texttt{T<B>}$$}
\end{mathpar}
\end{itemize}
